...
impl fmt::Write for KernelDebugWriter {
    fn write_str(&mut self, message: &str) -> fmt::Result {
        unsafe {
            let ptr = kernel_malloc::malloc((message.len() + 1) as raw::c_size_t,
                                     &mut kernel_malloc::M_DEVBUF[0],
                                     kernel_malloc::M_WAITOK as i32) as
                      *mut raw::c_char;
            if ptr.is_null() {
                ...
                kernel_malloc::uprintf(ptr);
                kernel_malloc::free(ptr as *mut raw::c_void, &mut kernel_malloc::M_DEVBUF[0]);
            }
        }
        Ok(())
    }
